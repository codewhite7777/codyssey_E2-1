...
### 1.1.0 이커머스란 무엇인가 — 정의와 본 프로젝트의 스코프

**이커머스(e-commerce)** 는 인터넷·모바일 등 **디지털 채널을 매개로 상품 또는 서비스를 거래하는 모든 상행위**를 말한다. 협의로는 _주문·결제_ 라는 거래 사건 자체를 의미하지만, 실제 산업은 **상품 기획 → 콘텐츠 제작 → 검색·노출 → 광고·마케팅 → 구매·결제 → 배송·CS → 리뷰·재구매**로 이어지는 전 가치사슬(end-to-end value chain)을 포괄한다.

국내 이커머스는 거래 주체와 채널 형태에 따라 다음과 같이 분류된다.

| 분류 축 | 유형 | 대표 사례 |
|---------|------|-----------|
| 거래 주체 | **B2C** (기업 → 소비자) | 쿠팡 · 11번가 · 아마존 |
| | **B2B** (기업 간) | 알리바바 · 이지웰비즈 |
| | **C2C** (개인 간) | 당근 · 번개장터 |
| | **B2B2C** (플랫폼 중개) | 네이버 스마트스토어 · 무신사 |
| 채널 형태 | 종합몰 · 전문몰(버티컬) · 소셜커머스 · **라이브커머스** · 메타커머스 | 쿠팡 · 무신사 · 카카오톡 선물하기 · 네이버 쇼핑라이브 · 다나와 |

본 프로젝트는 이 중 **B2C · B2B2C 채널에서 유통되는 건강기능식품 · 화장품 · 다이어트 상품**, 그 중에서도 **숏폼 동영상 플랫폼(YouTube Shorts · Instagram Reels · TikTok)으로 노출되는 광고** 에 초점을 둔다. 이 스코프를 벗어나는 영역(예: B2B 산업재·중고 거래 플랫폼·오프라인 연계 O2O)은 본 기획서 범위 밖이다.

> 🔎 **상세 레퍼런스** — 거래 주체별/채널별 전체 분류, 가치사슬 11개 레이어(L1 소싱~L11 재구매), 이해관계자×레이어 페인포인트 매트릭스는 [`docs/research/01_domain_definition.md`](../research/01_domain_definition.md) 에 정리되어 있다.
