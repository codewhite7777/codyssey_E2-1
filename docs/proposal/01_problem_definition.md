...
- **영상 기반 소비는 10~30대를 넘어 전 연령대로 확장**되고 있으며, 특히 고연령층에서 유튜브가 주력 정보 소비 창구로 자리잡았다 — 아래 연령대별 차트가 그 증거다.
...
> 아래 범위는 §1.1~§1.4 논의를 거쳐 팀이 확정한 사항이다 (상세 해설 [§1.4.1](#141-개입-좌표-시각화--l4--l6-교차점) 참조). 기획서의 §2(타겟 사용자) 이후 모든 섹션이 이 범위를 전제로 작성된다.
